...
export default function AlertsPage() {
  ...
  const handleAddAlert = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!keyword) return;
    if (channels.length === 0) { setError('Pilih minimal satu saluran notifikasi.'); return; }
    setSubmitting(true);
    setError('');
    try {
        ...
        <form onSubmit={handleAddAlert} className="p-6 rounded-xl border border-border bg-card space-y-4 h-fit">
          ...
          <div>
            ...
            <input type="text" placeholder="Contoh: Cisco, Kubernetes, Asphalt" value={keyword}
              ...
              className="w-full bg-input border border-border rounded-lg px-3 py-2 text-sm text-foreground placeholder-muted-foreground focus:outline-none focus:border-ring" required />
          </div>
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1.5">Saluran Notifikasi</label>
          ...
          {channels.includes('EMAIL') && (
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1.5">Alamat Email Pengiriman</label>
          ...
          {channels.includes('TELEGRAM') && (
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1.5">Chat ID Telegram</label>
          ...
          <button type="submit" disabled={submitting}
            className="w-full bg-maroon hover:bg-maroon-dark text-white font-semibold py-2 text-sm rounded-lg transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
            {submitting ? <span className="h-4 w-4 border-2 border-border border-t-transparent rounded-full animate-spin" /> : 'Aktifkan Alarm Pemantau'}
          </button>
        </form>
            ...
            {loading ? (
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-neutral-800 text-xs text-foreground">
                  {alerts.length === 0 ? (
                    <tr><td colSpan={4} className="p-8 text-center text-muted-foreground">Belum ada keyword terdaftar.</td></tr>
                  ) : (
                    alerts.map((alert) => (
                      <tr key={alert.id} className="hover:bg-maroon-darker/30">
                        <td className="p-4 font-medium text-foreground">{alert.keyword}</td>
                        <td className="p-4">
